...
getwd()
{
  ...
  nnc_data$credit_amount = as.numeric(nnc_data$credit_amount)
...
##### Convert more variables
ca_int = c("poi","resident_since", "credits_no", "liables")
{
  ## Train
  nnc_data[ca_int] = lapply(nnc_data[ca_int], factor)
  sapply(nnc_data, class)
  
  ## Test
  nnc_res[ca_int] = lapply(nnc_res[ca_int], factor)
  sapply(nnc_res, class)
}
...
#test_class = nn_ctest$credit_amount
{
  ...
  nnc_data = cbind(nnc_data, class.ind(as.factor(nnc_data$poi)))
  names(nnc_data)
  setnames(nnc_data, old = c("1","2","3","4"), 
           new = c("poi1","poi2", "poi3","poi4"))
  nnc_data$poi = NULL
  
  #c("poi","resident_since", "credits_no", "liables")
  nnc_data = cbind(nnc_data, class.ind(as.factor(nnc_data$resident_since)))
  names(nnc_data)
  setnames(nnc_data, old = c("1","2","3","4"), 
           new = c("resident_since1","resident_since2",
                   "resident_since3","resident_since4"))
  nnc_data$resident_since = NULL
  
  nnc_data = cbind(nnc_data, class.ind(as.factor(nnc_data$credits_no)))
  names(nnc_data)
  setnames(nnc_data, old = c("1","2","3","4"), 
           new = c("credits_no1","credits_no2",
                   "credits_no","credits_no4"))
  nnc_data$credits_no = NULL
  
  nnc_data = cbind(nnc_data, class.ind(as.factor(nnc_data$liables)))
  names(nnc_data)
  setnames(nnc_data, old = c("1","2"), 
           new = c("liables1","liables2"))
  nnc_data$liables = NULL
...
{
  nnc_res = cbind(nnc_res, class.ind(as.factor(nnc_res$poi)))
  names(nnc_res)
  setnames(nnc_res, old = c("1","2","3","4"), 
           new = c("poi1","poi2", "poi3","poi4"))
  nnc_res$poi = NULL
  
  #c("poi","resident_since", "credits_no", "liables")
  nnc_res = cbind(nnc_res, class.ind(as.factor(nnc_res$resident_since)))
  names(nnc_res)
  setnames(nnc_res, old = c("1","2","3","4"), 
           new = c("resident_since1","resident_since2",
                   "resident_since3","resident_since4"))
  nnc_res$resident_since = NULL
  
  nnc_res = cbind(nnc_res, class.ind(as.factor(nnc_res$credits_no)))
  names(nnc_res)
  setnames(nnc_res, old = c("1","2","3","4"), 
           new = c("credits_no1","credits_no2",
                   "credits_no","credits_no4"))
  nnc_res$credits_no = NULL
  
  nnc_res = cbind(nnc_res, class.ind(as.factor(nnc_res$liables)))
  names(nnc_res)
  setnames(nnc_res, old = c("1","2"), 
           new = c("liables1","liables2"))
  nnc_res$liables = NULL
...
xgbm = xgboost(data = data.matrix(nnc_data[,nX[!nX %in% nY]]),
               label = nnc_data$credit_amount,
               nrounds = 217)



## XXXXXXGBM Predit #######
pred_xgbm = predict(xgbm, data.matrix(nnc_res[,-1]))

pred_xgbm = round(pred_xgbm)

nnc_res$Predit_XGBM = pred_xgbm
names(nnc_res)
write.csv(nnc_res[, c('serial.number', 'Predit_XGBM')], "new_XGBM.csv", row.names = F)
